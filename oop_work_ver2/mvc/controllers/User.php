...
class User extends Controller
{
   ...
   public function user_create()
   {
      if ($_SERVER['REQUEST_METHOD'] === 'GET') {
         ...
         $strBirth = null;
         $pattern_birth="/^(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\\1|(?:(?:29|30)(\/|-|\.)(?:0?[13-9]|1[0-2])\\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/|-|\.)0?2\\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/|-|\.)(?:(?:0?[1-9])|(?:1[0-2]))\\4(?:(?:1[6-9]|[2-9]\d)?\d{2})$/";
         if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (empty($_POST["birth"])) {
               $dataError['err_birth'] = "Please enter your birth";
            } else {
              if (!preg_match($pattern_birth,$birth)) {
               $dataError['err_birth'] = "your type of birth is wrong";
              }
              else
              {
               $parts = explode("-", $birth);
         ...
         if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (empty($_POST["email"])) {
              ...
              if (!preg_match($pattern_email,$email)) {
               $dataError['err_email'] = "your type of email is wrong";
              }
            }
